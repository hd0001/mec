    ...
    def _try_update_sm(self):
        ...
        for zappi in state.zappi_list():
            ...
            if sm.should_health_charge() and zappi.mode == 'Eco+':
                ...
                self.sessions[zappi.sno]['low_charge'] = False
            elif zappi.mode not in ('Fast', 'Stop') and sm.should_stop_charge() and not zappi.boost_active():
                ...
                if zappi.sno in self.auto_eco:
                    if 'iBoost' in state._values and state._values['iBoost'] > 100:
                        ...
                        if available_power < zappi.min_charge_rate_with_level():
                            self.log.info('Setting Zappi to eco+ %s avail: %d rate: %d', zappi.sno, available_power, zappi.charge_rate)
                            self.server_conn.set_mode_ecop(zappi.sno)
